...
const MessageCommandsHandler = async function (client, message) {
    ...
    const prefixes = [...client.luhux.Prefix, `<@${client.user.id}>`, `<@!${client.user.id}>`];
    ...
    if (cmd) {
        ...
        const botCommandChannels = luhux.settings.botCommandChannels;
        if (botCommandChannels &&
            botCommandChannels.length > 0 &&
            !botCommandChannels.includes(message.channel.id) &&
            !message.member.permissions.has(PermissionsBitField.Flags.Administrator) &&
            !luhux.settings.founders.some((role) => message.member.roles.cache.has(role)) &&
            cmd.Name !== 'sil' &&
            cmd.Name !== 'tag' &&
            cmd.Name !== 'snipe' 
        ...
        const canExecuteData = Object.keys(luhux.cmdPerms || {}).some((key) => key === cmd.Category) ? luhux.cmdPerms[cmd.Category] : null;
        ...
        try {
            ...
            await cmd.messageRun(client, message, args, luhux, embed);
        } catch (err) {
