...
module.exports = class Staff {
  ...
  static check(member, luhux) {
    return member.roles.cache.some((r) => [...luhux.staffRanks].some((rr) => rr.role === r.id));
  };

  static getRank(member, luhux) {
    if (!luhux.staffRanks.length) return { currentRank: undefined, newRank: undefined };

    const sortedRoles = luhux.staffRanks.sort((a, b) => a.place - b.place);
  ...
  static async checkRank(client, member, luhux, options = { type, amount, user, point }) {
    const { currentRank, newRank } = Staff.getRank(member, luhux);
    if (!currentRank) return;
    if (currentRank && currentRank.type !== 'sub') return Staff.checkTasks(client, member, luhux, options);
    ...
    if (currentRank && currentRank.type === 'sub' && options?.type) {
      ...
      const responsbilitys = luhux?.staffResponsibilities?.filter((x) => member.roles.cache.has(x.role));
...
if (!newRank || document.totalPoints < newRank.point) return;


    const now = Date.now();
if ((now - document.roleStartAt) < 1000 * 60 * 60 * 24 * 7) return;
  ...
  static async checkTasks(client, member, luhux, options = { type, amount, user }) {
