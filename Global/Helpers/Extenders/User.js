...
module.exports = Object.defineProperties(GuildMember.prototype, {
                ...
                embeds: [
                    new EmbedBuilder({
                        author: {
                            ...
                            icon_url: staff.user.displayAvatarURL({ forceStatic: true })
                        },  
                        description: `${this}, kişinin adı **${name}** olarak değiştirildi.\n\n${bold(gender === 'Man' ? 'ERKEK' : 'KADIN')} olarak kayıt edildi.`,
                        footer: {
                            text: 'Kayıt işlemi başarıyla tamamlandı.',
                            icon_url: message.guild.iconURL({ dynamic: true, size: 1024 })
                          }
                    })
                ],
                components: [
                    new ActionRowBuilder().addComponents(
                        new ButtonBuilder().setCustomId(`yenikayıtmesaj`).setLabel('Kayıt İşlemi Başarılı!').setStyle(ButtonStyle.Secondary).setEmoji('✅').setDisabled(true),
    ...
    punish: {
        value: async function ({ type, message, question, luhux, timing, reason, image }) {
            ...
            const actions = {
                ...
                'Underworld': async () => {
                    ...
                    if (this.manageable) await this.setRoles(luhux.settings.underworldRole);
                },

                'Quarantine': async () => {
                    if (this.voice.channel) await this.voice.disconnect().catch(() => null);
                    if (this.manageable) await this.setRoles(luhux.settings.quarantineRole);
                },

                'Ads': async () => {
                    if (this.voice.channel) await this.voice.disconnect().catch(() => null);
                    if (this.manageable) await this.setRoles(luhux.settings.adsRole);
                },

                'VoiceMute': async () => {
                    if (this.voice.channel) await this.voice.setMute(true);
                    if (this.manageable) await this.roles.add(luhux.settings.voiceMuteRole);
                },

                'ChatMute': async () => {
                    if (this.manageable) await this.roles.add(luhux.settings.chatMuteRole);
                },

                'Event': async () => {
                    if (this.manageable) await this.roles.add(luhux.settings.eventPenaltyRole);
                },

                'Streamer': async () => {
                    if (this.manageable) await this.roles.add(luhux.settings.streamerPenaltyRole);
                }
            }
