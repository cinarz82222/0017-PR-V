...
module.exports = async function channelDelete(client, guild = Guild.prototype, audit = GuildAuditLogsEntry.prototype, member, changes, luhux) {
	const safeMode = await checkWhitelist(client, member, 'channel');
	if (safeMode?.isWarn && !luhux.blackListedChannels.includes(audit?.targetId)) {
