...
module.exports = async function channelUpdate(client, guild, audit, member, changes, luhux) {
	const safeMode = await checkWhitelist(client, member, 'channel');
	if (safeMode?.isWarn) return;
	if (safeMode?.isWarn && !luhux.blackListedChannels.includes(audit?.targetId)) return;
