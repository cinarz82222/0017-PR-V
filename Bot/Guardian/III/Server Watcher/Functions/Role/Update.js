...
module.exports = async function roleUpdate(client, guild, audit, member, changes, luhux) {
	const safeMode = await checkWhitelist(client, member, 'role');
	if (safeMode?.isWarn) return;
	if (safeMode?.isWarn && !luhux.blackListedRoles.includes(audit?.targetId)) return;
