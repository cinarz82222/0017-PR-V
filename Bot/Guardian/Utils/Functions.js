...
async function punish(client, member, type, reason, action) {
	...
	switch (type) {
		case 1:
			member.guild.bans.create(member.id, { reason: 'Adel Security - ' + reason }).catch(() => { });
			break;

		case 2:
			member.kick('Adel Security - ' + reason).catch(() => { });
			break;
...
const logChannel = client.channels.cache.get('1384177722875707486'); 
       const affectedRoles = dangerRoles.map(r => `<@&${r.id}>`).join(', ') || 'Bulunamadı';
if (logChannel) {
    logChannel.send({
...
embeds: [
  {
    title: 'Sunucu Güvenliği Devreye Alındı',
    description: `Sunucudaki tehlikeli tüm rollerin yetkileri geçici olarak kapatıldı.`,
    fields: [
      { name: 'Sebep', value: reason || 'Belirtilmedi', inline: true },
      { name: 'Etkilenen Roller', value: affectedRoles, inline: false },
    ],
...
async function checkWhitelist(client, member, type) {
	...
	const whitelistType = guildOwner ? '👑 Taç Sahibi ' : fullAccess ? '🟢 Full Erişim' : developers ? `👑 Developer` : bots ? `🤖 Adel Bots` : `🟢 Güvenli Listede`
...
async function createChatGuardian(client, id) {
	try {
		...
		if (moderations.some(x => x.name.includes('luhux'))) return;
