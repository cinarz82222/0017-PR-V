...
module.exports = {
    ...
    messageRun: async (client, message, args, luhux, embed) => {
        if (!(luhux.settings.unregisterRoles || []).length) {
            client.embed(message, 'Sunucuda kayıtsız rolü ayarlanmamış.');
            return;
        };

        const limit = client.functions.checkLimit(message.author.id, 'Unregister', luhux.settings.unregisteredLimitCount || 5, ms('5m'));
        if (limit.hasLimit) {
        ...
        message.reply({
            embeds: [
                embed.setDescription(
                    `${member} (${inlineCode(member.id)}) kullanıcısına ${luhux?.settings?.unregisterRoles.map(r => roleMention(r)).listArray()} rolleri verildi!`,
                ),
            ],
        });
