...
module.exports = {
    ...
    messageRun: async (client, message, args, luhux) => {
        ...
        const row = new ActionRowBuilder({
            components: [
                new ButtonBuilder({
                    ...
                    disabled: luhux.settings.manRoles.some(x => member.roles.cache.has(x))
                }),
                new ButtonBuilder({
                    custom_id: 'woman',
                    label: 'Kadın',
                    style: ButtonStyle.Secondary,
                    disabled: luhux.settings.womanRoles.some(x => member.roles.cache.has(x))
                }),
            ],
        });
        ...
        collector.on('collect', async (i) => {
            ...
            await member.roles.remove(i.customId === 'man' ? luhux.settings.womanRoles : luhux.settings.manRoles).catch(err => { });
            await member.roles.add(i.customId === 'man' ? luhux.settings.manRoles : luhux.settings.womanRoles).catch(err => { });
