...
module.exports = {
    ...
    messageRun: async (client, message, args, luhux) => {
        ...
        const serverPosition = Array.from(message.guild.members.cache.values())
        ...
        const section2 = new SectionBuilder()
            .addTextDisplayComponents(new TextDisplayBuilder().setContent(
                [
                    ...
                    `- Sunucu Pozisyonu: ${bold(serverPosition)}. (Sonraki: ${message.guild.members.cache.find(m =>
                        Array.from(message.guild.members.cache.values())
                            .sort((a, b) => a.joinedAt - b.joinedAt)
                            .map(m => m.id)
                            .indexOf(m.id) === serverPosition) || 'Bulunamadı.'})`,
                    `- Bazı Rolleri;`,
                    `➥ ${roleList}`,
                ].join('\n')
            ))
