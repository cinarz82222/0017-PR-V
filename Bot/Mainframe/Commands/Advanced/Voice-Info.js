...
module.exports = {
    ...
    messageRun: async (client, message, args, luhux, embed) => {
        ...
        const taggedMembers = voiceMembers.filter(m => m.user.displayName.includes(luhux.settings.tag));
        ...
        const publicMembers = voiceMembers.filter(m => m.voice.channel.parentId === luhux.settings.publicParent);
        const streamerMembers = voiceMembers.filter(m => m.voice.channel.parentId === luhux.settings.streamerParent);
        const registerMembers = voiceMembers.filter(m => m.voice.channel.parentId === luhux.settings.registerParent);
        const secretMembers = voiceMembers.filter(m => m.voice.channel.parentId === luhux.settings.privateRoomParent);
        const funParents = voiceMembers.filter(m => m.voice.channel.parentId === luhux.settings.activityParent);
