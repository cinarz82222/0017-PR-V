    ...
    messageRun: async (client, message, args, luhux, embed) => {
        ...
        const staffs = await Promise.all(
            document.staffs.map(async (s, i) => {
                ...
                return `${inlineCode(` ${i + 1}. `)} ${staff ? staff : '[@bulunamadı](https://luhux.live)'} ${staff ? client.staff.check(staff, luhux) ? await client.getEmoji('check') : await client.getEmoji('mark') : await client.getEmoji('mark')} - ${client.timestamp(s.date)}`;
            })
        );
