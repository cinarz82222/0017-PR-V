...
module.exports = {
    ...
    messageRun: async (client, message, args, luhux, embed) => {
        ...
        if (!client.staff.check(member, luhux)) return message.channel.send({ content: `${member.id === message.author.id ? 'Yetkili değilsiniz.' : 'Belirttiğin kullanıcı yetkili değil!'}` });
        ...
        if (!timing && !reason.length) {
            ...
            collector.on('collect', async (i) => {
                ...
                collector.stop('luhux');
            });

            collector.on('end', (_, reason) => {
                if (reason === 'luhux') return;
