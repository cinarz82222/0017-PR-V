    ...
    messageRun: async (client, message, args, luhux, embed) => {
        ...
        const taggeds = await Promise.all(
            document.taggeds.map(async (t) => {
                ...
                return `${tagged ? tagged : '[@bulunamadı](https://luhux.live)'} ${tagged ? tagged?.tag() ? await client.getEmoji('check') : await client.getEmoji('mark') : await client.getEmoji('mark')} - ${client.timestamp(t.date)}`;
            })
        );
