    ...
    messageRun: async (client, message, args, luhux, embed) => {
        ...
        if (!client.staff.check(member, luhux)) {
            ...
            const mappedData = document.oldRanks.map((r) => {
                const role = r.roles.find(role => luhux.staffRanks.some((rr) => rr.role === role));
                const date = client.timestamp(r.date);

                return `[${date}]: ${role && message.guild?.roles.cache.has(role) ? message.guild.roles.cache.get(role) : '[@bulunamadı](https://luhux.live)'}`;
            });
            ...
            collector.on('collect', async (i) => {
                ...
                if (i.customId === 'accept') {
                    ...
                    const sortedRanks = luhux.staffRanks.filter(x => x.type === 'sub').sort((a, b) => a.point - b.point);
                            ...
                            embeds: [
                                new EmbedBuilder({
                                    ...
                                    description: [
                                        ...
                                        `→ Yetki veren: ${message.author}`,
                                        `→ Yetki verme tarihi: ${client.timestamp(now)}`,
                                        `→ Verilen Yetkiler: ${roleMention(sortedRoles[0].role)} ${sortedRoles[0].hammers.map(h => roleMention(h)).join(' ')}`,
                                        `→ Hesap oluşturma tarihi: ${client.timestamp(member.user.createdTimestamp)}`,
                                        `→ Sunucuya katılma tarihi: ${client.timestamp(member.joinedTimestamp)}`,
                                    ].join('\n'),
                                })
                    ...
                    await client.staff.checkRank(client, message.member, luhux, { type: 'staffPoints', amount: 1, user: member.id, point: 50 });
                } else {
        ...
        } else if (luhux.settings?.staffUpdateAuth.some(r => message.member?.roles.cache.has(r)) || message.member.permissions.has(PermissionFlagsBits.Administrator)) {
            const document = await StaffModel.findOne({ user: member.id }) || { oldRanks: [] };
            ...
            const question = await message.channel.send({
                components: message.member.permissions.has(PermissionFlagsBits.Administrator) ? [subRow, middleRow, topRow]
                    : luhux.settings.founders.some(r => message.member.roles.cache.has(r)) ? [subRow, middleRow, topRow]
            ...
            collector.on('collect', async (i) => {
                const newRank = luhux.staffRanks.find((r) => r.role === i.values[0]);
