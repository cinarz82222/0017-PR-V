...
module.exports = {
    ...
    messageRun: async (client, message, args, luhux, embed) => {
        ...
        if (!client.staff.check(member, luhux)) return message.channel.send({ content: 'Belirttiğin kullanıcı yetkili değil!' });
        ...
        const { currentRank, currentIndex } = client.staff.getRank(member, luhux);
        ...
        const oldRanks = await Promise.all(
            staffDocument.oldRanks.reverse().slice(0, 5).map(async (r) => {
                ...
                return `${await client.getEmoji('point')} [${client.timestamp(r.date)}] ${r.up ? await client.getEmoji('check') : await client.getEmoji('mark')} : ${staff ? staff : '[@bulunamadı](https://luhux.live)'} ${staff ? (role ? roleMention(role) : '[@bulunamadı](https://luhux.live)') : '[@bulunamadı](https://luhux.live)'} => ${r.reason}`;
            })
        );
        ...
        const excuses = await Promise.all(
            (staffDocument?.excuses || []).map(async (e) => {
                ...
                return `${await client.getEmoji('point')} [${client.timestamp(e.startAt)} - ${client.timestamp(e.endAt)}] ${staff ? staff : '[@bulunamadı](https://luhux.live)'} : ${e.reason}`;
            })
        );
        ...
        const ilisuion = new EmbedBuilder({
            ...
            description: [
                ...
                `${await client.getEmoji('point')} Yetkiyi Veren: ${firstStaffStarter ? firstStaffStarter : '[@bulunamadı](https://luhux.live)'}`,
                ...
                `${await client.getEmoji('point')} Yetkiyi Veren: ${lastStaffStarter ? lastStaffStarter : '[@bulunamadı](https://luhux.live)'}`,
