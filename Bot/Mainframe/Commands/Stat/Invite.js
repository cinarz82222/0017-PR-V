        ...
        const row = new ActionRowBuilder({
            components: [
                ...
                new ButtonBuilder({
                    ...
                    disabled: (invitedUsers.length < 1 || !message.guild?.roles.cache.has(luhux.settings.minStaffRole)),
                })
            ]
        });
            ...
            invitedUsers.map(async (x) => {
                ...
                if (member)
                    datax.push([
                        ...
                        member ? member.roles.highest.position >= message.guild?.roles.cache.get(luhux.settings.minStaffRole)?.position ? 'Yetkili' : 'Değil' : 'Sunucuda değil',
