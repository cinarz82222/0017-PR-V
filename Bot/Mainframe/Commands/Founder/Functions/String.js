...
module.exports = async function String(client, message, option, luhux, question, author, menu = 'main', functionType) {
    await question.edit({
        components: createRow(option.name, luhux[option.root][option.value], option.isMultiple),
    });
    ...
    collector.on('collect', async (i) => {
        ...
        if (i.isButton() && i.customId === 'change') {
                ...
                components: [
                    new TextInputBuilder({
                        ...
                        value: option.isMultiple
                            ? (luhux[option.root][option.value] || []).slice(0, 60).join(', ') || undefined
                            : (luhux[option.root][option.value].toString() || '').slice(0, 60) || undefined,
                        label: 'Yeni Ayar:',
                        maxLength: 60,
                        required: true,
            ...
            if (modalCollector) {
                ...
                if (option.isMultiple) luhux[option.root][option.value] = value.split(',').map((v) => v.trim());
                    ...
                    luhux[option.root][option.value] = Number(value);
                } else luhux[option.root][option.value] = value;

                await message.guild?.updateSettings({
                    $set: { [`${option.root}.${option.value}`]: luhux[option.root][option.value] },
                });
        ...
        if (i.isButton() && i.customId === 'reset') {
            luhux[option.root][option.value] = undefined;

            await message.guild?.updateSettings({
                $unset: { [`${option.root}.${option.value}`]: luhux[option.root][option.value] },
            });
