...
module.exports = async function Role(client, message, option, luhux, question, author, menu = 'main', functionType) {
    await question.edit({
        ...
        components: createRow(client, message.guild, luhux?.staffRanks || []),
    });
    ...
    collector.on('collect', async (i) => {
        ...
        if (i.isRoleSelectMenu()) {
            if (i.customId === 'roleAdd') {
                ...
                if (luhux.staffRanks?.some((r) => r.role === rankRoleId)) {
                    return i.reply({
                        ...
                        if (type === 'sub') {
                            ...
                            if (modalCollected) {
                                ...
                                const newRank = [
                                    ...(luhux.staffRanks || []),
                                    {
                                        place: luhux.staffRanks.length + 1,
                                        type: type,
                                        role: rankRoleId,
                                        hammers: hammers,
                                        point: point,
                                ...
                                await SettingsModel.updateOne({ id: luhux.id }, { staffRanks: newRank });
        ...
        if (i.isStringSelectMenu()) {
            ...
            const newData = (luhux.staffRanks || []);
            luhux.staffRanks = newData.filter((d) => !i.values.includes(d.role));
            ...
            await message.guild?.updateSettings({ staffRanks: luhux.staffRanks });
        };
    });
}
