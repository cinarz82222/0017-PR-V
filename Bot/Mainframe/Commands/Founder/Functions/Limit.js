...
module.exports = async function Limit(client, message, option, luhux, question, author, menu = 'main', functionType) {
    await question.edit({
        content: '',
        components: createRow(
            option.name,
            luhux[option.root][`${option.value}Limit`],
        ),
    });
    ...
    collector.on('collect', async (i) => {
        ...
        if (i.isButton() && i.customId === 'change') {
            const limitValue = String(luhux[option.root]?.[`${option.value}Limit`] ?? '');
            ...
            if (modalCollector) {
                luhux[option.root][`${option.value}Limit`] = modalCollector.fields.getTextInputValue('count');

                await i.guild?.updateSettings({
                    $set: {
                        [`${option.root}.${option.value}Limit`]: luhux[option.root][`${option.value}Limit`],
                    },
                });

                modalCollector.reply({
                    content: `Başarıyla ${bold(option.name)} adlı ayar ${inlineCode(luhux[option.root][`${option.value}Limit`])} şeklinde ayarlandı.`,
                    ephemeral: true,
                });
        ...
        if (i.isButton() && i.customId === 'reset') {
            luhux[option.root][`${option.value}Limit`] = undefined;
