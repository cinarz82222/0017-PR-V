...
module.exports = async function Komut(client, question, type, luhux, message, menu, functionType) {
    ...
    collector.on('collect', async (i) => {
        ...
        if (i.customId === 'manage') {
            const commandPerms = luhux.cmdPerms || {};
        ...
        if (i.isAnySelectMenu() && i.values.length) {
            ...
            luhux.cmdPerms = luhux.cmdPerms || {};
            luhux.cmdPerms[categoryKey] = luhux.cmdPerms[categoryKey] || { access: [], commands: [] };
            luhux.cmdPerms[categoryKey].commands = luhux.cmdPerms[categoryKey].commands || [];

            const data = luhux.cmdPerms[categoryKey].commands.find((c) => c.command === commandName);
            ...
            if (accessCollected) {
                ...
                const access = accessCollected.values;
                const Access = (luhux.cmdPerms[categoryKey].commands.find((c) => c.command === cmd.Name)?.access || []);
                const newPermissions = [];
                ...
                luhux.cmdPerms[categoryKey].commands = luhux.cmdPerms[categoryKey].commands.filter((c) => c.command !== cmd.Name);
                luhux.cmdPerms[categoryKey].commands.push({
                    command: cmd.Name,
                    access: newPermissions,
                });

                await SettingsModel.updateOne({ id: message.guild.id }, { $set: { 'cmdPerms': luhux.cmdPerms } });
