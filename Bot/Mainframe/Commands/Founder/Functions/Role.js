...
module.exports = async function Role(client, message, option, luhux, question, author, menu = 'main', functionType) {
    await question.edit({
        components: createRow(client, message.guild, option, luhux[option.root][option.value])
    });
    ...
    collector.on('collect', async (i) => {
        ...
        if (i.isRoleSelectMenu()) {
            if (option.isMultiple) luhux[option.root][option.value] = i.values;
            else luhux[option.root][option.value] = i.values[0];

            await i.guild?.updateSettings({
                [`${option.root}.${option.value}`]: luhux[option.root][option.value]
            });
        ...
        if (i.isStringSelectMenu()) {
            if (option.isMultiple) {
                const newData = luhux[option.root][option.value] || [];
                luhux[option.root][option.value] = newData.filter((r) => !i.values.includes(r));
            } else luhux[option.root][option.value] = undefined;
...
function createRow(client, guild, option, data) {
    const chunks = client.functions.chunkArray((option.isMultiple ? data : [data]).filter((r) => guild.roles.cache.has(r)), 25);
    const rows = [];
    let page = 0;
