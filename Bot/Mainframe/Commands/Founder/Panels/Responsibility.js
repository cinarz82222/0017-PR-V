...
module.exports = {
    ...
    messageRun: async (client, message, args, luhux) => {
        if (!luhux.settings.staffResponsibilities.length) return message.channel.send({ content: 'Sunucuda hiçbir sorumluluk tanımlanmamış.' });

        message.channel.send({
            components: createRow(client, message.guild, luhux.settings.staffResponsibilities),
            content: [
            `## Merhaba ${bold(inlineCode(message.guild?.name || 'luhux'))} sorumluluk seçim paneline hoşgeldiniz.`,
            ...
            luhux.staffRanks.filter(r => r.type === 'sub' && message.guild?.roles.cache.has(r.role)).map(r => message.guild?.roles.cache.get(r.role)).listArray(),

            `- Yetkiniz bu kapsamda ise, minimum ${bold('1')} ve maksimum ${bold('3')} sorumluluk üstlenebilirsiniz.`,
            luhux.staffRanks.filter(r => r.type === 'middle' && message.guild?.roles.cache.has(r.role)).map(r => message.guild?.roles.cache.get(r.role)).listArray(),

            `- Yetkiniz bu kapsamda ise, minimum ${bold('2')} ve maksimum ${bold('3')} sorumluluk üstlenebilirsiniz.`,
            luhux.staffRanks.filter(r => r.type === 'top' && message.guild?.roles.cache.has(r.role)).map(r => message.guild?.roles.cache.get(r.role)).listArray(),
