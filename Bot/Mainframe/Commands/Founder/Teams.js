...
module.exports = {
    ...
    messageRun: async (client, message, args, luhux) => {
        const teams = (luhux.teams || []).filter((t) => message.guild?.roles.cache.has(t.role));
        ...
        collector.on('collect', async (i) => {
            ...
            if (i.customId === 'team-list') {
                ...
                collector2.on('collect', async (i) => {
                    ...
                    const team = luhux.teams.find((team) => team.name.toLowerCase() === i.values[0]);
                            ...
                            new EmbedBuilder({
                                ...
                                description: codeBlock(
                                    ...
                                    [
                                        ...
                                        `→ Sunucu Taglı Üye Sayısı: ${teamMembers.filter((m) => luhux.settings.tag.includes(m.user.displayName)).size}`,
                    ...
                    collector3.on('collect', async (i) => {
                        ...
                        luhux.teams = luhux.teams.filter((t) => t.name !== team.name);
            ...
            if (i.customId === 'team-ranking') {
                ...
                const teamStats = teams.map(team => {
                    ...
                    return {
                        ...
                        taggedMembers: teamMembers.filter((m) => luhux.settings.tag.includes(m.user.displayName)).size,
                ...
                const attachment = new AttachmentBuilder(buffer, { name: 'luhux-teams.png' });

                const embed = new EmbedBuilder({
                    color: client.getColor('random'),
                    title: 'Ekip Ses Aktivite Sıralaması',
                    image: { url: 'attachment://luhux-teams.png' },
