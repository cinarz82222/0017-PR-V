...
module.exports = {
    ...
    messageRun: async (client, message, args, luhux) => {
        if (!message.guild?.roles.cache.has(luhux.settings.meetingRole)) {
        ...
        if (collected) {
            ...
            const staffMembers = channel?.members.filter((m) => client.staff.check(m, luhux)).map((m) => m.id);

            if (collected.customId === 'general') {
                if (luhux.settings.meetingRole && message.guild?.roles.cache.has(luhux.settings.meetingRole)) {
                    message.guild.members.cache
                        .filter(
                            (member) =>
                                !channel?.members.has(member.id) && member.roles.cache.has(luhux.settings.meetingRole),
                        )
                        .forEach((member) => member.roles.remove(luhux.settings.meetingRole));
                    channel?.members
                        .filter((member) => !member.user.bot && !member.roles.cache.has(luhux.settings.meetingRole))
                        .forEach((member) => member.roles.add(luhux.settings.meetingRole));
                }
