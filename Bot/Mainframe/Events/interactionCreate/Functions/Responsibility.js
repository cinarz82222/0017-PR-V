...
module.exports = async function Responsibility(client, interaction, route, luhux) {
    ...
    if (!isNaN(Number(route))) {
        ...
        const currentStaffResponsibilityRoles = member.roles.cache.filter(role =>
            luhux.settings.staffResponsibilities.includes(role.id)
        );
        ...
        if (logChannel) logChannel.send({
            ...
            embeds: [
                new EmbedBuilder({
                    footer: { text: `${member.user.username} ${member.id}` },
