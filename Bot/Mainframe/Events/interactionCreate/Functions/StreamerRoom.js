...
module.exports = async function StreamerRoom(client, interaction, route, luhux) {
    const member = interaction.guild?.members.cache.get(interaction.user.id)
    if (!member) return;

    if (!member.permissions.has(PermissionFlagsBits.Administrator) && !member.roles.cache.has(luhux.settings.streamerRole)) {
        interaction.reply({ content: `Bu işlemi gerçekleştirmek için ${roleMention(luhux.settings.streamerRole)} rolüne sahip olmalısınız.`, ephemeral: true });
        return;
    }
    ...
    if (channel.parentId !== luhux.settings.streamerParent) {
        interaction.reply({ content: 'Streamer odalarında bulunmuyorsunuz.', ephemeral: true });
        return;
    }

    const hasAnotherOwner = luhux.streamerRooms.find((c) => c.channel === channel.id);
    const owneredChannel = luhux.streamerRooms.find(
    ...
    if (route === 'owner') {
        ...
        collector.on('collect', async i => {
            if (i.customId === 'select') {
                ...
                if (!target.permissions.has(PermissionFlagsBits.Administrator) && !target.roles.cache.has(luhux.settings.streamerRole)) {
                    interaction.reply({ content: `Belirttiğin kişide ${roleMention(luhux.settings.streamerRole)} rolü bulunmamaktadır.`, ephemeral: true });
                    return;
                }
    ...
    if (route === 'settings') {
        ...
        const streamerRole = interaction.guild.roles.cache.get(luhux.settings.streamerRole);
