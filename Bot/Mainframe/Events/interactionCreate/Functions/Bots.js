...
module.exports = async function Bots(client, interaction, route, luhux) {
    if (route === 'select') {
        const value = interaction.values[0];
        if (!value) return interaction.reply({ content: `${await client.getEmoji('mark')} Bir bot seçimi yapmalısın.`, ephemeral: true });

        const botClient = global.luhuxBots.Main.find((c) => c.id === value) || global.luhuxBots.Welcome.find((c) => c.id === value);
    ...
    if (route === 'avatar') {
        ...
        const Client = global.luhuxBots.Main.find((client) => client.id === bot.id) || global.luhuxBots.Welcome.find((client) => client.id === bot.id);
