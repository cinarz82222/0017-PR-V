...
module.exports = async function Punitive(client, interaction, route, id, luhux) {
    ...
    if (route === 'false') {
        ...
        if (penal.type === 'ChatMute') {
            user.roles.remove(luhux.settings.chatMuteRole);
        };

        if (penal.type === 'VoiceMute') {
            user.roles.remove(luhux.settings.voiceMuteRole);
        };

        if (penal.type === 'Event') {
            user.roles.remove(luhux.settings.eventPenaltyRole);
        }

        if (penal.type === 'Streamer') {
            user.roles.remove(luhux.settings.streamerPenaltyRole);
        }
