...
module.exports = async function Streamer(client, interaction, route, luhux) {
    if (route === 'appeal') {
        if (!luhux.settings.streamerRole) return interaction.reply({ content: 'Streamer rolü ayarlanmamış.', ephemeral: true });

        const member = interaction.guild?.members.cache.get(interaction.user.id);
        if (member.roles.cache.has(luhux.settings.streamerRole)) return interaction.reply({ content: 'Zaten streamer rolüne sahipsiniz.', ephemeral: true });
    ...
    if (route === 'modal') {
        ...
        member.roles.add(luhux.settings.streamerRole).catch(() => null);
        ...
        if (channel) channel.send({
            embeds: [
                new EmbedBuilder({
                    ...
                    footer: { text: `${interaction.guild.name} | Created By luhux` },
                    timestamp: new Date()
                })
            ]
        });
