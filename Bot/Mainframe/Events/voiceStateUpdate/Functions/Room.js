...
module.exports = async function roomHandler(client, oldState, newState, luhux) {
        if (newState.channelId === luhux?.settings?.privateRoomChannel) {
        const member = newState.guild.members.cache.get(newState.id);
        const roomControl = (luhux?.privateRooms || []).find(x => x.owner === member.id);
        if (roomControl) {
        ...
        luhux?.settings.womanRoles.forEach((role) => {
            perms.push({
                id: role,
        ...
        luhux?.settings.manRoles.forEach((role) => {
            perms.push({
                id: role,
                allow: [PermissionFlagsBits.ViewChannel]
            })
        ...
        luhux?.settings.unregisterRoles.forEach((role) => {
            perms.push({
                id: role,
