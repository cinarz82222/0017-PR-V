...
module.exports = {
    ...
    execute: async (client, oldMessage, newMessage) => {
        ...
        for (const chunk of oldChunks) {
            const embeds = [new EmbedBuilder().setTitle('Eski Mesaj Resimleri').setURL('https://luhux.live')];

            for (const img of chunk) {
                embeds.push(new EmbedBuilder({
                    url: 'https://luhux.live',
                    image: {
                        url: img,
                    },
                }));
        ...
        for (const chunk of newChunks) {
            const embeds = [new EmbedBuilder().setTitle('Yeni Mesaj Resimleri').setURL('https://luhux.live')];
