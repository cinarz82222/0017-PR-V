...
module.exports = async function BannedTag(client, member, luhux, channel) {
    const tags = luhux.settings.bannedTags || [];
    if (!tags.length) return false;

    const tag = tags.find((t) => member.user.displayName.toLowerCase().includes(t.toLowerCase()));
    if (!tag) return false;

    await member.setRoles(luhux.settings.bannedTagRole).catch(() => null);
    ...
    if (bannedTagLog) bannedTagLog.send({
        ...
        embeds: [new EmbedBuilder({
            ...
            description: `${member} adlı üye yasaklı taglı (${inlineCode(tag || 'luhuxcumHarikasın')}) olduğu için banlandı`,
            fields: [
                {
                    name: '\u200b',
