...
module.exports = async function Tag(client, oldUser, newUser, member, luhux) {
    const oldHasTag = luhux.settings.tag.split('').some((t) => oldUser.displayName.includes(t))
    const newHasTag = luhux.settings.tag.split('').some((t) => newUser.displayName.includes(t))
    const tagMemberCount = member.guild.members.cache.filter((m) => m.user.displayName.includes(luhux.settings.tag))
    ...
    if (!oldHasTag && newHasTag) {
        member.roles.add(luhux.settings.familyRole)
        setTimeout(() => {
            member.setNickname(member.displayName.replace(luhux.settings.secondTag, luhux.settings.tag)).catch(() => { })
        }, 2000)
    ...
    if (oldHasTag && !newHasTag) {
        ...
        if (luhux.systems.taggedMode === true && !member.premiumSince) {
            member.setRoles(luhux.settings.unregisterRoles).catch(() => { })
            member.setNickname(`${luhux.settings.secondTag} ${luhux.settings.name}`).catch(() => { })
            ...
            member.roles.remove(luhux.settings.familyRole)
            setTimeout(() => {
                member.setNickname(member.displayName.replace(luhux.settings.tag, luhux.settings.secondTag)).catch(() => { })
            }, 2000)
            setTimeout(async() => {
